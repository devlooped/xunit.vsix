		...
		public bool ShouldRestart ()
		{
			...
			try {
				...
				if (ex.GetType ().Name == "NotInstalledException") {
					...
					var vsixPath = Path.Combine (Path.GetTempPath (), @"Xamarin\xunit.vsix", rootSuffix + ".vsix");
					if (!File.Exists (vsixPath))
						throw new InvalidOperationException ("Did not find the Xunit VSIX extension installer at " + vsixPath + ".");
